...
pub fn execute_effect(effect: &Effect, context: &mut Game) -> Result<Vec<Action>> {
    ...
    match effect {
        ...
        Effect::IncreaseAbsoluteMaxMana {
            ...
            let increase_absolute_max_mana_actions =
                mana::compute_increase_absolute_max_mana(context, initiator, player, amount)?;
            actions.extend(increase_absolute_max_mana_actions);
        }
        Effect::Boost {
            initiator,
            attack,
