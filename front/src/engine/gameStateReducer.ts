...
export const applyAction = (state: IGameState, action: IAction): IGameState => {
  switch (action.type) {
    ...
    case "DecreaseCurrentMana":
      return applyDecreaseMana(state, action);
    case "Win":
...
function applyDecreaseMana(state: IGameState, action: Extract<IAction, { type: "DecreaseCurrentMana" }>): IGameState {
  if (action.value.player === state.player.hero.id) {
    return {
      ...state,
      player: {
